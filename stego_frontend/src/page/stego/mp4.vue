<template>
    <div class="sys-page">
        ...
        <div class="page-content">
            <app-notes>
                <p>读取视频文件：使用视频处理库FFmpeg加载MP4文件，分离视频流和音频流。</p>
                <p>选择嵌入位置：确定秘密信息嵌入的位置，视频帧的特定区域，或者音频流的某些部分。</p>
                <p>编码秘密信息：将秘密信息编码为适合嵌入到视频或音频流中的格式，例如，使用二进制数据或特定的信号模式。</p>
                <p>嵌入信息到视频：通过修改像素值的最低有效位（LSB），在视频帧的特定像素或元数据中嵌入秘密信息。</p>
            </app-notes>
            <app-section title="mp4视频嵌入">
                <div class="sys-article">
                    <p><strong>上传说明</strong></p>
                    <p>mp4视频嵌入只支持上传mp4视频作为载体，txt文本文件存放隐藏信息</p>
                    <p>载体大小为5M的条件下，可嵌入大小为4552B。嵌入率为0.09%</p>
                </div>
                <el-row :gutter="20">
                  <el-col :span="12">
                    <div class="sys-article">
            ...
            <app-section title="mp4视频提取">
              <div class="sys-article">
                    <p><strong>上传说明</strong></p>
                    <p>mp4视频提取只支持上传mp4视频进行提取</p>
                </div>
                <el-row :gutter="20">
                  <el-col :span="12">
                    <div class="sys-article">
                    ...
                    <el-upload
                        ...
                        accept=".mp4"
                        :file-list="fileList3"
                        :on-success="handleSuccessMP42"
                        :on-remove="handleRemoveMP42"
                        multiple
                        :limit="1">
                        <i class="el-icon-upload"></i>
                        <div class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
  ...
  export default {
    ...
    methods: {
      handleSuccessMP42(response, file, fileList){
        console.log(response.filename)
        this.fileNameMP42 = response.filename
      },
      handleRemoveMP42(file, fileList) {
        this.fileNameMP42 = null;
      },
      handleRemoveMP4(file, fileList) {
        this.fileNameMP4 = null;
      },
      handleRemoveTXT(file, fileList) {
        this.fileNameTXT = null;
      },
      handleSuccessMP4(response, file, fileList){
        console.log(response.filename)
        this.fileNameMP4 = response.filename
      },
      ...
      embed(){
        console.log(this.fileNameMP4)
        console.log(this.fileNameTXT)
  
        // 检查fileJPG是否为null
        if (this.fileNameMP4 === null) {
            alert("请先上传载体图像");
        ...
        // 使用axios
        // 使用axios发送POST请求
        axios.post('/api/embedMP4', {
          fileNameMP4: that.fileNameMP4, // 你的请求参数
        ...
        .then(response => {
          ...
          reader.onload = (e) => {
            try {
              ...
              link.href = url;
              link.setAttribute('download', 'stegoMP4.mp4'); // 指定下载的文件名
      ...
      extract() {
        ...
        axios.post('/api/extractMP4', {
          fileNameMP4: this.fileNameMP42,
          userId: this.userId
        }, {
