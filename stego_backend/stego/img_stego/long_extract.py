...
# 读取图片进行分割
input_image_path = "merged_image.jpg"
output_folder = "examples/split/split_secret"
split_size = 256
split_images = split_image(input_image_path, output_folder, split_size)
...
for image_file in image_files:
    ...
    if(image_file == "tile_0_0.jpg"):
        ...
        print(f'嵌入信息的图片数字符串为：{secret_decoded[0]}')
        image_num = int(secret_decoded[0])
        print(f'嵌入信息的图片数为：{image_num}')
    else:
        if(i < image_num):
            secret_decoded = extract_img(image_path)
...
print(len(all_recovered_secrets))
