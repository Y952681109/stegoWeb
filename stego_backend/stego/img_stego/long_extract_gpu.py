...
if len(sys.argv) != 3:
    print("使用方法: python long_extract_test.py <stegoJPG_path> <outTXT_path>")
    sys.exit(1)
    
    # 获取命令行参数
stegoJPG_path = sys.argv[1]
outTXT_path = sys.argv[2]


input_image_path = stegoJPG_path
secret = extract_long_words(input_image_path)
print("decode secrets:")
print(secret)

with open(outTXT_path, 'w', encoding='utf-8') as file:
    file.write(secret)