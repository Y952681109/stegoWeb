...
class SecretEncoder3(nn.Module):
    ...
    def forward(self, x, c):
        ...
        # print(f'输入张量形状: {c.size()}')
        # x = self.secret_scaler[0](c)
        # print(f'经过nn.Linear后的形状: {x.size()}')
        # x = self.secret_scaler[1](x)
        # print(f'经过nn.SiLU后的形状: {x.size()}')
        # x = self.secret_scaler[2](x)
        # print(f'经过View后的形状: {x.size()}')
        # x = self.secret_scaler[3](x)
        # print(f'经过nn.Upsample后的形状: {x.size()}')
        # x = self.secret_scaler[4](x)
        # print(f'经过卷积层后的形状: {x.size()}')
        # print("------------------------------------------------------------------------------------")

        # print(f'原嵌入信息的形状: {c.size()}')
        c = self.encode(c)
        # print(f'编码后嵌入信息的形状: {c.size()}')
        return c, None
